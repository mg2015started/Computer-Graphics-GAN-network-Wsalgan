...
from models.model_w_salgan import ModelWSALGAN
from utils import *

flag = str(sys.argv[1])


def w_salgan_batch_iterator(model, train_data, validation_sample):
    num_epochs = 301
    n_updates = 1
...
def train():
    ...
    # Create network
    if flag == 'wsalgan':
        model = ModelWSALGAN(INPUT_SIZE[0], INPUT_SIZE[1])
        # Load a pre-trained model
        # load_weights(net=model.net['output'], path='test/gen_', epochtoload=15)
        w_salgan_batch_iterator()(model, train_data, validation_sample.image.data)
    else:
